...
extern void exceptionNotifier(int id);
...
extern "C" void hard_fault_handler_c (unsigned int * hardfault_args)
{
  ...
  while (1)
  {
      ...
      log ("R3 = %x\n", stacked_r3);
      log ("R12 = %x\n", stacked_r12);
      log ("LR [R14] = %x  subroutine call return address\n", stacked_lr);
      log ("PC [R15] = %x  program counter\n", stacked_pc);
      log ("PSR = %x\n", stacked_psr);
...
void SysTick_Handler(void)
{
    ...
    {
        extern void sysTickDivBy100Handler();
        sysTickDivBy100Handler();
    }
}
...
void delay_tiks(uint32_t tiks)
{
    uint32_t t;
    t = tiks + getCurrentTicks();
    while(t > getCurrentTicks()){};
}
